...
gulp.task('watch-scss', ['browser-sync'], () => {
...
gulp.task('watch', () => {
  ...
  gulp.watch(
    [`${paths.dev}/js/**/*.js`, 'js/**/*.js', '!js/theme.js', '!js/theme.min.js'],
    ['scripts']
  );
 ...
 * @verbose
 */
gulp.task('imagemin-watch', ['imagemin'], done => {
  browserSync.reload();
  done();
});
...
gulp.task('watch-bs', ['browser-sync', 'watch', 'scripts'], () => {});
...
gulp.task('clean-source', () => del(['src/**/*']));
...
gulp.task('clean-dist-product', () => del([`${paths.distprod}/**`]));
